...
# of minimal even rankability distance) that join separate components.
...
def _dist_query(tree, data, max_neighbors_search):
    return tree.query(data, k=max_neighbors_search, dualtree=True, breadth_first=True, sort_results = True)
...
cdef class KDTreeBoruvkaAlgorithm (object):
    ...
    leaf_size : int, optional (default=20)
        ...
        is used for a kNN search for rank distance. A second tree is
    ...
    n_jobs : int, optional (default=4)
        The number of parallel jobs used to compute rank distances.
    ...
    **kwargs :
    ...
    cdef np.double_t *even_rank_distance_ptr
    cdef np.double_t *bounds_ptr

    cdef np.ndarray components
    cdef np.ndarray even_rank_distance_arr
    ...
    def __init__(self, tree, max_neighbors_search=16, min_flatting=0, metric='euclidean', leaf_size=20,
        ...
        self.even_rank_distance_ptr = <np.double_t *> &self.even_rank_distance[0]
        self.bounds_ptr = <np.double_t *> &self.bounds[0]

    cdef _compute_bounds(self):
        """Initialize even rankability distances"""
        ...
        self.even_rank_distance_arr = knn_dist[:, self.max_neighbors_search - 1].copy()
        self.even_rank_distance = (<np.double_t[:self.num_points:1]> (
            <np.double_t *> self.even_rank_distance_arr.data))

        if self.min_flatting >= self.max_neighbors_search:
            # this is how hdbscan works
            for n in range(self.num_points):
                for i in range(1, self.max_neighbors_search + 1):
                    m = knn_indices[n, i]
                    if self.even_rank_distance[m] <= self.even_rank_distance[n]:
                        self.candidate_point[n] = n
                        self.candidate_neighbor[n] = m
                        self.candidate_distance[n] = self.even_rank_distance[n]
                        break
        else:
            # DRUHG
            for n in range(self.num_points):
                self.even_rank_distance[n] = <np.double_t> DBL_MAX

            # Since we already computed NN distances for the max_neighbors_search closest
            # points we can use this to do the first round of boruvka -- we won't
            # get every point due to even_rank distance
            # issues, but we'll get quite a few, and they are the hard ones to
            # get, so fill in any we can and then run update components.
            for n in range(self.num_points):
                i = self.max_neighbors_search
                while i != 0:
                    m = knn_indices[n, i]
                    # i is the rank of m for n
                    # let j the rank of n for m

                    j = i
                    if knn_dist[m, j] <= knn_dist[n, i]:
                        j = self.max_neighbors_search - 1
                        while knn_dist[m, j] > knn_dist[n, i]:
                            j -= 1
                    if j < self.min_flatting:
                        j = self.min_flatting

                    if knn_dist[n, j] < self.even_rank_distance[n]:
                        self.even_rank_distance[n] = knn_dist[n, j]
                        self.candidate_point[n] = n
                        self.candidate_neighbor[n] = m
                        self.candidate_distance[n] = knn_dist[n, j]

                    if knn_dist[m, j] < self.even_rank_distance[n]:
                        self.even_rank_distance[n] = knn_dist[m, j]
                        self.candidate_point[n] = n
                        self.candidate_neighbor[n] = m
                        self.candidate_distance[n] = knn_dist[m, j]

                    i -= 1
##############################

        # Since we do everything in terms of rdist to free up the GIL
        # we need to convert all the rank distances beforehand
        # to make comparison feasible.
        for n in range(self.num_points):
            self.even_rank_distance[n] = self.dist._dist_to_rdist(
                self.even_rank_distance[n])
            self.candidate_distance[n] = self.dist._dist_to_rdist(
                self.candidate_distance[n])
    ...
    cdef int update_components(self) except -1:
        ...
        # Since we're working with even rankability distance we often have
    ...
    cdef int dual_tree_traversal(self, np.intp_t node1,
        ...
        #           - if the even rank distance for a point is larger than
        #             the distance to the nearest neighbor of the
        #             component of the point ... then we can't get
        #             a better even rank distance and we
        ...
        #           - we need to recompute even rank distance
        #             because some points are not each other knn neighbors
        ...
        if node1_info.is_leaf and node2_info.is_leaf:
            ...
            for i in range(point_indices1.shape[0]):
                ...
                if (self.even_rank_distance_ptr[p] >
                ...
                for j in range(point_indices2.shape[0]):
                    ...
                    if (self.even_rank_distance_ptr[q] >
                            self.candidate_distance_ptr[component1]):
                        continue
                    ...
                    if component1 != component2:
                        ...
                        #               self.even_rank_distance_ptr[p],
                        #               self.even_rank_distance_ptr[q])
                        if self.alpha != 1.0:
                            mr_dist = max(d / self.alpha,
                                          self.even_rank_distance_ptr[p],
                                          self.even_rank_distance_ptr[q])
                        else:
                            mr_dist = max(d, self.even_rank_distance_ptr[p],
                                          self.even_rank_distance_ptr[q])
...
cdef class BallTreeBoruvkaAlgorithm (object):
    ...
    leaf_size : int, optional (default=20)
        The Boruvka algorithm benefits from a smaller leaf size than
        standard kd-tree nearest neighbor searches. The tree passed in
        is used for a kNN search for rank distances. A second tree is
        constructed with a smaller leaf size for Boruvka; this is that
        leaf size.
    ...
    **kwargs :
    ...
    def __init__(self, tree, max_neighbors_search=16, min_flatting=0,  metric='euclidean',
...
#######################################
    ...
    cdef int dual_tree_traversal(self, np.intp_t node1,
        ...
        #
        #       We get to take some shortcuts:
        #           - if the rank distance for a point is larger than
        #             the distance to the nearst neighbor of the
        #             component of the point ... then we can't get
        #             a better even rankability distance and we
        #             can skip computing anything for that point
        #           - if the points are in the same component we
        #             don't have to compute the distance.
        #
        #       We also have some catches:
        #           - we need to compute even rankability distance
        #             not just the ordinary distance; this involves
        #             fiddling with rank distances.
        #           - We need to scale distances according to alpha,
        #             but don't want to lose performance in the case
        #             that alpha is 1.0.
        #
        ...
        if node1_info.is_leaf and node2_info.is_leaf:
            ...
            for i in range(point_indices1.shape[0]):
                ...
                if self.even_rank_distance_ptr[p] > self.candidate_distance_ptr[
                        component1]:
                    continue
                ...
                for j in range(point_indices2.shape[0]):
                    ...
                    if self.even_rank_distance_ptr[q] > self.candidate_distance_ptr[
                            component1]:
                        continue
