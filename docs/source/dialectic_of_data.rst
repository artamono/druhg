...
.. table:: Что происходит между K и L?
   ...
   | "Глазами" К                                                  |  "Глазами" L                               |
   +==============================================================+============================================+
   | **Если K будет считать** ближних к себе, то получится так.   | **Если же L начнёт считать** от себя, то:  |
...
| Вспомним нашего субъекта K, он был независим, но тем не менее, он видел более близких LM. Для того чтобы выравнять свои отношения, субъектам необходимо "ждать" чужих.
   ...
   .. figure:: ./pics/first/QualityReciprocated.png
      ...
      Подсчёт выражения из А в М. В окружностях должно быть по 4 объекта, это увеличит расстояние до :math:`D_{AL}`. В одной окружности 2 своих, а в чужой 3. Мера :math:`\sqrt{\frac{2}{3}}`.
...
|
| Общность станет кластером, если её внутренние различия будут преодолены :math:`$$\sum_{K} N_{i} · D^2_{i} <= K · D^2_r · \sqrt{min(N, N')}$$`.
